...
#extra code taken from http://www.sthda.com/english/wiki/ggplot2-quick-correlation-matrix-heatmap-r-software-and-data-visualization

#built in heatmap
heatmap(correlation_matrix, Colv = NA, Rowv = NA, scale = "none")

# Get lower triangle of the correlation matrix
lower_tri_cor<-function(cormat){
  cormat[lower.tri(cormat)] <- NA
  return(cormat)
}

heatmap(lower_tri_cor(correlation_matrix), Colv = NA, Rowv = NA, scale = "none")
...
melted_cor <- melt(lower_tri_cor(correlation_matrix), na.rm = TRUE)
...
#following code manipulated from https://www.datanovia.com/en/blog/easy-correlation-matrix-analysis-in-r-using-corrr-package/
install.packages("corrr")
require(corrr)

cor <- correlate(air_clean, method = "spearman", diagonal = 1)
cor %>% rearrange() %>% shave() %>% rplot(shape = 15, colours = c("red","blue"))
```
